...
46. テーブル付きページを開くとフィルタバー右端に「コピー」ボタンが表示される
47. `<table data-no-filter>` 属性付きテーブルにはコピーボタンが出ない
48. クリックするとクリップボードに CSV（`,` 区切り / ヘッダー含む）が入る
49. Shift+クリックで Markdown テーブル形式（`|` 区切り / 区切り行あり）がコピーされる
50. フィルタで絞り込んだ状態でコピー → 表示行のみ＋ヘッダーがコピーされる（非表示行は除外）
51. ソート後にコピー → 表示順がそのまま CSV / Markdown に反映される
52. ヘッダーの矢印（⇅ ▲ ▼）はコピー結果に含まれない
53. ハイライト（`<mark>`）が付いている状態でコピーしてもテキストにマークは含まれない
54. セルに `,` `"` を含むデータ → CSV がダブルクォートで正しくエスケープされる
55. セルに `|` を含むデータ → Markdown 出力が `\|` にエスケープされる
56. コピー成功時にボタンテキストが「✓ CSVコピー済み」または「✓ MDコピー済み」に変わり 2 秒後に「コピー」に戻る
57. クリップボード API が拒否された場合はボタンに「コピー失敗」が 2 秒表示される
58. 編集モードへ移行するとコピーボタンも消える（フィルタバーごと cleanup）
59. 編集モードから戻るとコピーボタンが再生成される
60. 印刷プレビューでコピーボタンが非表示になる
61. プラグイン無効化（`unmount`）でコピーボタン・イベントリスナ・タイマーが全解除される
