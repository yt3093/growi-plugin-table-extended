...
| スティッキーヘッダー | スクロール時に `thead` を画面上部に固定。`top` は CSS 変数 `--gpte-sticky-top` で上書き可能（デフォルト `0px`）。固定時に薄い下方向 `box-shadow` を付与 |
...
27. `--gpte-sticky-top: 60px` を CSS で指定するとヘッダーが 60px 下に固定される
28. `<table data-no-sticky>` 属性付きテーブルでは sticky にならない（ソート/フィルタは有効）
29. 編集モード移行時は sticky も解除され、閲覧モード復帰で再度固定される
30. 印刷プレビューでスティッキーが解除され（`position: static`）、影も消える
31. プラグイン無効化で `gpte-sticky-head` クラスが全テーブルから削除される
