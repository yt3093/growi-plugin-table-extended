...
| フィルタ | テーブル上部に検索ボックス 1 つ。全列対象・大小文字無視の部分一致。スペース区切りで AND 絞り込み。入力欄左端にじょうごう型 SVG アイコンを描画（`pointer-events: none`）、placeholder テキストは廃止 |
...
46. フィルタバーの入力欄に placeholder テキストが表示されない
47. 入力欄の左端にじょうごう型（funnel）SVG アイコンが表示される
48. アイコン部分をクリックしても入力欄にフォーカスが当たり、文字入力ができる（`pointer-events: none`）
49. 入力欄にフォーカスを当てるとじょうごうアイコン色が primary 色に変わる
50. スクリーンリーダーが `aria-label="テーブルをフィルタ"` で入力欄の用途を読み上げる（SVG は `aria-hidden`）
51. ダークモード切替でじょうごうアイコン色のコントラストが保たれる
52. プラグイン無効化でじょうごうアイコンも含めフィルタバーが DOM から完全に消える
53. 印刷プレビューでフィルタバー（じょうごうアイコン含む）が非表示になる
54. テーブル付きページを開くとフィルタバー右端に「コピー」ボタンが表示される
